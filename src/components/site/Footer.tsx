    ...
    <footer className="border-t border-white/10 bg-ink text-white">
      <div className="mx-auto max-w-7xl px-5 py-16 sm:px-8">
        ...
        <div className="mt-14 flex flex-col gap-3 border-t border-white/10 pt-6 text-xs text-white/45 sm:flex-row sm:items-center sm:justify-between">
          <div>© {new Date().getFullYear()} {clinic.legalName}. All rights reserved.</div>
          <div className="flex gap-5">
            <a href="#" className="transition-colors duration-200 hover:text-vet-green">Privacy</a>
            <a href="#" className="transition-colors duration-200 hover:text-vet-green">Terms</a>
          </div>
        </div>
      </div>
    </footer>
      ...
      <ul className="mt-5 space-y-3">
        {links.map((link) => (
          <li key={`${title}-${link.label}`}>
            <a
              href={link.href}
              className="text-sm text-white/75 transition-colors duration-200 hover:text-vet-green"
            >
              {link.label}
            </a>
          </li>
